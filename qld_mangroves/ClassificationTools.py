...
from utils import SpatialTools
    ...
    for train_shp in train_shps:
        ...
        if training_gpd[train_field].dtype != np.int64 and not train_reclass:
            ...
            meta = {'crs': str(training_xarray_all.crs.crs_wkt),
                    ...
                    'affine': training_xarray_all.crs.GeoTransform,
                    'width': cols_train,
                    'height': rows_train,
                    'driver': 'GTiff',
                    'count': 1,
            ...
            transform, projection = SpatialTools.geotransform(training_xarray, (training_xarray.x, training_xarray.y), epsg=3577)
            rows,cols = (len(training_xarray.x), len(training_xarray.y))
            training_pixels = SpatialTools.rasterize_vector(train_shp, cols=cols, rows=rows, 
                                               geo_transform=transform, projection=projection, 
                                               field=train_field, raster_path='temp_raster.tif')
...
def randomforest_classify(classifier, analysis_data, classification_output, class_prob=False):
    ...
    geo_transform, proj = SpatialTools.geotransform(analysis_data, (analysis_data.x, analysis_data.y), epsg=3577)
    # Covert to array and rearrange dimension order
    ...
    # Export to file
    SpatialTools.array_to_geotiff(classification_output,
                     data=classification,
    ...
    if class_prob:
        ...
        # Export to file
        SpatialTools.array_to_geotiff(splitext(classification_output)[0] + "_prob.tif",
